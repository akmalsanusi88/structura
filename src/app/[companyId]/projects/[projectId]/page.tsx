...
import type { PlantUnit, InHouseTeam, Company, Project, PurchaseOrder, DailyActivityLog, Claim, TeamCost, MaterialRequisition, MaterialIssuance, MaterialReturn, DailyActivityWork, SiteInstruction, DeliveryOrder, MaterialOnSiteUsage, OtherCost, Contract } from "@/lib/types";

export default async function ProjectDetailsPage({ params }: { params: Promise<{ companyId: string, projectId: string }> }) {
  ...
  const [
    ...
    supabase.from('other_costs').select('*').eq('project_id', projectId),
          ...
          subconClientClaims.forEach(claim => {
              ...
              if (mainPoId && mainContractorPo) {
                  syncedClaims.push({
                      ...
                      sstPercentage: claim.sst_percentage,
                      sstAmount: claim.sst_amount,
  ...
  const project: Project = {
    ...
    otherCosts: (otherCostsResult.data || []).map((c: any): OtherCost => ({
      id: c.id,
      ...
      quotationNo: c.quotation_no,
      purchaseOrderNo: c.purchase_order_no,
  ...
  const contracts: Contract[] = (contractsResult.data || []).map((c: any) => ({
    id: c.id,
    companyId: c.company_id,
